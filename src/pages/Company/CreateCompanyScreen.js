...
import {
    ...
    BreadcrumbItem, Breadcrumb, Button, InputGroup, Table
} from "reactstrap";
// Redux
...
import {createCompany, getCompany, getState, getCity, getCnae, getTax, clearMessage} from "../../store/actions";
...
class CreateCompanyScreen extends Component {
    ...
    tog_standard() {
        this.setState(prevState => ({
            alert_modal_standard: !prevState.alert_modal_standard
        }));
    }

    show_dialog = (alert_modal_message) => {
        this.setState({alert_modal_standard: true});
        this.setState({ alert_modal_message });
    }
    ...
    handlePrimary = (value) => {
        this.setState({primary: value.value})
    }
    ...
    onClickCreate = () => {
        ...
        if ( this.state.secondaryItems.length === 0){
            this.show_dialog("Deve existir pelo menos um CNAE secundário");
        }
        else if ( this.state.taxRegimes.length === 0){
            this.show_dialog("Deve existir pelo menos um Regime tributário para concluir o cadastro.");
        }
        else{
            const company = {
            ...
            this.props.createCompany(company, this.props.history);
        }
    }

    validate = (cnpj, company, nome, address, complement, email, cep, neighborhood, primary) => {
        return {
            cnpj: cnpj.length === 0,
    ...
    render() {
        ...
        const {secondaryItems, taxRegimes} = this.state;
                    ...
                    <Container fluid>
                                        ...
                                        <Row style={{alignItems: 'center', marginTop: "1em"}}>
                                            <Col lg="8">
                                                <FormGroup className="mr-2">
                                                    ...
                                                    <Input
                                                        className={errors.nome ? "form-control error" : "form-control"}
                                                        type="search"
                                        ...
                                        <Row style={{alignItems: 'center', marginTop: "1em"}}>
                                            <Col lg="4">
                                                <FormGroup className="mr-2">
                                                    <Label>Bairro (*)</Label>
                                                    <Input
                                                        className={errors.neighborhood ? "form-control error" : "form-control"}
                                                        type="search"
                                            ...
                                            <Col lg="4">
                                                ...
                                                <FormGroup className="select2-container">
                                                    <Input
                                                        className={errors.cep ? "form-control error" : "form-control"}
                                                        type="search"
                        ...
                        <Row>
                            <Col sm={6} md={4} xl={3} className="mt-4">
                                ...
                                <Modal
                                    isOpen={this.state.alert_modal_standard}
                                    toggle={this.tog_standard}
                                >
                                    <ModalBody>
                                        <h5>{this.state.alert_modal_message}</h5>
                                    </ModalBody>
                                    <ModalFooter>
                                        <Button
                                            type="button"
...
const mapStatetoProps = state => {
    ...
    const {tax} = state.Tax;
    return {states, cities, cnaes, tax}
}
